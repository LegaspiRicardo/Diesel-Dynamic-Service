...
export default function SimpleHeroCarousel({ slides, autoPlayInterval = 4000 }: SimpleHeroCarouselProps) {
    const [currentSlide, setCurrentSlide] = useState(0);
    const [nextSlideIndex, setNextSlideIndex] = useState(1);
    const [isTransitioning, setIsTransitioning] = useState(false);

    const TRANSITION_DURATION = 800; // Aumentado para más suavidad

    const nextSlide = useCallback(() => {
        if (isTransitioning) return;
        setIsTransitioning(true);
        
        // Prepara el próximo slide
        const nextIndex = (currentSlide + 1) % slides.length;
        setNextSlideIndex(nextIndex);
        
        setTimeout(() => {
            setCurrentSlide(nextIndex);
            setNextSlideIndex((nextIndex + 1) % slides.length);
    ...
    const prevSlide = useCallback(() => {
        ...
        setTimeout(() => {
            ...
            setNextSlideIndex((prevIndex + 1) % slides.length);
            setIsTransitioning(false);
        }, TRANSITION_DURATION);
    }, [isTransitioning, currentSlide, slides.length]);
    ...
    const goToSlide = useCallback((index: number) => {
        ...
        setTimeout(() => {
            ...
            setNextSlideIndex((index + 1) % slides.length);
            setIsTransitioning(false);
        }, TRANSITION_DURATION);
    }, [isTransitioning, currentSlide, slides.length]);
            ...
            <div className="relative h-full w-full">
                ...
                >
                    ...
                    <div className={`absolute inset-0 flex flex-col justify-center items-center px-4 transition-all duration-500 ${
                        ...
                        <a
                            href={`https://api.whatsapp.com/send?phone=523320853721&text=${encodeURIComponent(`Hola, solicito información sobre: ${slides[currentSlide].title}`)}`}
                            target="_blank"
                ...
                >
                    ...
                    <div className={`absolute inset-0 flex flex-col justify-center items-center px-4 transition-all duration-500 ${
                        ...
                        <a
                            href={`https://api.whatsapp.com/send?phone=523320853721&text=${encodeURIComponent(`Hola, solicito información sobre: ${slides[nextSlideIndex].title}`)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="bg-red-800/90 hover:bg-yellow-600 text-white font-semibold px-8 py-4 rounded-lg transition-all duration-300 text-lg hover:scale-105 transform"
                        >
